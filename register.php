...
if(isset($_POST['submit'])){
    ...
    $userCheckSql = "SELECT * FROM data WHERE username = '$username'";
    $resultCheck = mysqli_query($conn, $userCheckSql);
    if(mysqli_num_rows($resultCheck)>0){
        $errorMsg = "This Username already Exist !!";
    }else{  
        move_uploaded_file($tmp_name,'uploads/'.$filename);
        
        $sql = "INSERT INTO data(name, username, password, role, user_image) VALUES ('$name', '$username', '$password', '$role', '$filename')";
        $result = mysqli_query($conn, $sql);
        
        if($result){
            $successMsg = "Your account has been successfully created";
        }else{
            $errorMsg = "Got some issue in creating an account. Please enter valid information !!";
        }
    }
}
?>
    ...
    <div class="container mt-4">
        <form method="post" enctype="multipart/form-data">
            ...
            if($errorMsg){
                ...
                ?>
            <div class="col-md-5">
                <h3 class="reg-heading">Registration Form:</h3>
            </div>
            ...
            <button type="submit" class="btn btn-primary mt-3 col-md-1" name="submit">Submit</button>
            <div class="form-text div_down col-md-5">Already have an account? <a href="login.php">Login</a></div>
        </form>
    </div>
